<template>
  <el-upload :auto-upload="false" :show-file-list="false" accept=".pdf,.csv,.xls,.xlsx,image/*" @change="onAdd" class="flex justify-end mb-4">
    <el-button type="primary" :disabled="!$hasPermission('purchases.update')">
  ...
  <div v-loading="loading" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
    ...
    <div v-else v-for="(doc, index) in documents" :key="index" :title="doc.customMetadata?.fileName" class="relative group aspect-square rounded-lg overflow-hidden border bg-gray-100 border-gray-200">
      ...
      <el-button text type="danger" class="absolute top-1 right-1" size="small" circle :disabled="!$hasPermission('purchases.update')" @click="remove(doc.key)">
...
import PurchaseApi from '@/modules/purchases/api.ts';
...
const load = async () => {
  try {
    loading.value = true;
    const response = await PurchaseApi.getDocuments(props.uid);
...
const onAdd = async (uploadFile: UploadFile) => {
  ...
  try {
    loading.value = true;
    await PurchaseApi.uploadDocument(props.uid, uploadFile.raw);
    await load();
...
const remove = async (document: string) => {
  ...
  try {
    loading.value = true;
    await PurchaseApi.deleteDocument(props.uid, document);
    await load();

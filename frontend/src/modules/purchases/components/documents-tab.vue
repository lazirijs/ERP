<template>
  ...
  <div v-loading="loading" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
    ...
    <div v-for="(doc, index) in documents" :key="index" class="relative group aspect-square rounded-lg overflow-hidden border bg-gray-100 border-gray-200">
      <a :href="$getFileUrl(doc)" target="_blank" rel="noopener" download class="block size-full">
        <img v-if="isImage(doc)" :src="$previewImage({ type: 'image', src: doc })" class="object-cover size-full" />
        <div v-else class="flex flex-col items-center justify-center size-full gap-2 text-gray-400">
          ...
          <span class="text-xs font-medium uppercase">{{ getFileExtension(doc) }}</span>
        </div>
      </a>
      <el-button text type="danger" class="absolute top-1 right-1" size="small" circle @click="remove(doc)">

<template>
    ...
    <div class="flex flex-col h-full overflow-hidden">
            ...
            >
                <template v-for="item in Const.menuItems" :key="item.name">
                    <el-menu-item v-if="!item.children" :index="item.routeName" @click="handleMenuItemClick(item.routeName)">
                        ...
                        <template #title>
                            <span :dir="$appStore().selectedLocale.dir" class="w-full">
                                {{ $t(item.name) }}
                            </span>
                        </template>
                    </el-menu-item>
        ...
        <footer :dir="$appStore().selectedLocale.dir" class="h-16 flex justify-center items-center text-center text-sm text-gray-500">
            {{ $t('version') }} 1.0.0
        </footer>
    </div>
    </aside>
